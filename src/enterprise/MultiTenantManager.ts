...
export class MultiTenantManager extends EventEmitter {
   ...
   */
  async createTenant(
    config: Omit<ITenantConfiguration, 'createdAt' | 'lastUpdated' | 'isActive'>,
  ...
  async deleteTenant(tenantId: string): Promise<boolean> {
    ...
    const tenantSessions = Array.from(this.sessions.values()).filter(
  ...
  async createSession(
    ...
    if (activeSessions.length >= maxSessions) {
      throw new Error(
        `Maximum concurrent sessions (${maxSessions}) reached for tenant ${tenantId}`,
      );
    }
  ...
  getActiveSessions(tenantId: string): ITenantSession[] {
    return Array.from(this.sessions.values()).filter(
      session => session.tenantId === tenantId && session.isActive,
    );
  }
  ...
  checkResourceLimits(tenantId: string, operation: IResourceOperation): IResourceCheckResult {
    ...
    // Check storage
    if (
      operation.storageRequired &&
      current.storageMb + operation.storageRequired > limits.maxStorageMb
    ) {
      return {
        allowed: false,
