...
export class InMemoryDatabaseProvider extends DatabaseProvider {
    ...
    async updateAssetMetadata(assetId: string, tenantId: string, updates: Partial<IAssetMetadata>): Promise<boolean> {
        const key = this.getAssetKey(assetId, tenantId);
    ...
    async searchAssets(options: IAssetSearchOptions): Promise<IAssetMetadata[]> {
        let results = Array.from(this.assets.values())
            .filter(asset => asset.tenantId === options.tenantId);
    ...
    async getPerformanceMetrics(tenantId: string, startDate: Date, endDate: Date): Promise<IPerformanceMetrics[]> {
    ...
    async getAnalytics(query: IAnalyticsQuery): Promise<IAnalyticsResult> {
        const metrics = await this.getPerformanceMetrics(query.tenantId, query.startDate, query.endDate);
        ...
        metrics.forEach(metric => {
            ...
            const existing = timeSeriesMap.get(key) || { operationCount: 0, totalDuration: 0, memoryUsage: 0 };
        ...
        const assetTypes = Array.from(this.assets.values())
            .filter(asset => asset.tenantId === query.tenantId);
    ...
    async deleteTenantData(tenantId: string): Promise<void> {
        ...
        const keysToDelete = Array.from(this.assets.keys()).filter(key => key.startsWith(tenantId + ':'));
        ...
        const depKeysToDelete = Array.from(this.dependencies.keys()).filter(key => key.startsWith(tenantId + ':'));
 ...
 */
export function createDatabaseIntegration(type: 'memory' | 'sqlite' | 'postgresql' = 'memory'): DatabaseIntegration {
