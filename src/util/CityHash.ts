...
export class CityHash {
  ...
  private static toLongLE(b: Buffer, i: number): long {
    return Long.fromNumber(b[i + 7], true)
      .shl(56)
  ...
  private static toIntLE(b: Buffer, i: number) {
    return (
      ((b[i + 3] & 255) << 24) + ((b[i + 2] & 255) << 16) + ((b[i + 1] & 255) << 8) + (b[i] & 255)
    );
  }
  ...
  private static hashLen16(u: long, v: long): Long {
  ...
  private static hashLen0to16(s: Buffer, pos: number, len: number): long {
    ...
    if (len >= 4) {
      ...
      return this.hashLen16Mul(
        Long.fromNumber(len + (a << 3), true),
        Long.fromNumber(this.fetch32(s, pos + len - 4), true),
        mul,
      );
    }
    if (len > 0) {
  ...
  private static hashLen17to32(s: Buffer, pos: number, len: number): long {
    ...
    return this.hashLen16Mul(
      this.rotate(a.add(b), 43).add(this.rotate(c, 30).add(d)),
      a.add(this.rotate(b.add(this.k2), 18)).add(c),
      mul,
    );
  }
  ...
  public static cityHash64(s: Buffer, pos: number, len: number): long {
    ...
    let z = this.hashLen16(
      this.fetch64(s, pos + len - 48).add(len),
      this.fetch64(s, pos + len - 24),
    );
    ...
    return this.hashLen16(
      this.hashLen16(v[0], w[0]).add(this.shiftMix(y).multiply(this.k1)).add(z),
  ...
  public static cityHash64WithSeeds(
    s: Buffer,
    pos: number,
    len: number,
    seed0: long,
